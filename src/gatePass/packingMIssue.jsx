...
function Packing_Issue() {
  ...
  const handleSaveImage = () => {
    ...
    if (ImageReceiver !== "" && receiverSign !== "") {
      commonService.ReceiverDetailsForPackingM(finalObject).then((x) => {
        setActiveStep(1);
        alertService.clear();
  ...
  const handleIssueRequest = () => {
    ...
    finalObject["CostCenterID"] = state.AllReqComInfo[0].CostCenterID;
    finalObject["SubCostCenterID"] = state.AllReqComInfo[0].SubCostCenterID;
    debugger;
    let finalArray = [];
    ...
    if(finalArray.length != 0){
      ...
      commonService.StoreIssueSavingPacking(finalObject).then((x) => {
        alertService.clear();
        alertService.success(x);
        debugger;
        actions.setIssueID(parseInt(x));
        setstepTwoLoading(true);
        commonService.StoreIssueNoteForPacking(state.issueHeaderID).then((x) => {
          debugger;
      ...
      reqDetails.forEach((item) => {
        if (item.ReqDetailID == values.id) {
          ...
          if (values.field == "IssueQty") {
            var v1 = parseFloat(parseFloat(item.ApprovedQty) -parseFloat(item.IssuedQty)).toFixed(2);
            if (values.value <= item.ApprovedQty && parseFloat(values.value).toFixed(2) <= parseFloat(item.SIH) && parseFloat(values.value) <= (parseFloat(v1))) {
              debugger;
  ...
  const handleChangeCompany = (event) => {
    ...
    commonService.GetReqCodeByCompanyForPackingM(state.company).then((x) => {
  ...
  const handleChangeReq = (event, value) => {
    ...
    commonService.GetReqDetailsForPackingM(state.reqCode).then((x) => {
      setReqDetails(x);
    });
    debugger;
  };
